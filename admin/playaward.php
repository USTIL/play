...
$adminplayid = $_SESSION['adminplayid'];
$sql = "select * from awards where up_id = {$adminplayid} and sh = 1 order by place";
$rst = $mysql->query($sql);
...
<div class="layui-layout layui-layout-admin">
  ...
  <div class="layui-body">
    <!-- 内容主体区域 -->
      ...
      $sql = "select * from plays where id = {$adminplayid}";
      $rst = $mysql->query($sql);
	...
	layui.use(['table','layer'], function(){
	  ...
	  layer = layui.layer;
		...
		var idwidth = dwidth*0.2;
		var titlewidth = dwidth*0.2;
		var contwidth = dwidth*0.2;
		var actionwidth = dwidth*0.2;
		var acwidth = dwidth*0.2;
		table.render({
			elem: '#table1',
			cols:  [[
		...
		table.on('tool(table1)', function(obj){
		  ...
		  var tr = obj.tr; //获得当前行 tr 的DOM对象
			...
			if (layEvent == 'del') { //删除 
				layer.confirm('确定要删除“'+data.name+'”的获奖信息吗？',{title: '提示'}, function(index){
