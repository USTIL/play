...
<div class="layui-layout layui-layout-admin">
  <div class="layui-header">
    <div class="layui-logo">后台 - 用户管理</div>
  ...
  <div class="layui-body">
	...
	layui.use(['table','layer'], function(){
	  ...
	  layer = layui.layer;
		...
		var titlewidth = dwidth*0.24;
		var contwidth = dwidth*0.33;
		var actionwidth = dwidth*0.33;
		table.render({
			elem: '#table1',
			cols:  [[
		...
		table.on('tool(table1)', function(obj){
		  ...
		  var tr = obj.tr; //获得当前行 tr 的DOM对象
			...
			if (layEvent == 'detail') { //查看
				$.post('action.php?action=uadminaward', {
					id: data.id
					}, function(data) {
						if (data == 'success') {
							location.reload();
			...
			layer.confirm('删除一个用户就会删除这个用户的所有信息，确定要删除用户“'+data.name+'”吗？',{title: '提示'}, function(index){
				$.post('action.php?action=deleteuser', {
				id: data.id
				}, function(data) {
					if (data == 'success') {
						location.reload();
	...
	<form class="layui-form layui-form-pane" action="">
		...
		<div class="layui-form-item">
			...
			<div class="layui-input-block">
				<input type="text" id="usernameme" name="username" placeholder="请输入用户名" autocomplete="off" class="layui-input">
			</div>
		</div>
		<div class="layui-form-item">
